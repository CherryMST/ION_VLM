...
public abstract class RecyclerBaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void notifySetDatas(T[] t) {
        setDatas(t);
        if (mDatas != null && mDatas.size() > 0) {
            notifyDataSetChanged();
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if ((mDatas == null || mDatas.isEmpty()) && showDataNull) {
            return;
        } else {
    ...
    public void setShowDataNull(boolean showDataNull) {
        this.showDataNull = showDataNull;
    }
