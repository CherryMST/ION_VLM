...
/**
 * 类似桌面的workspace控件
 * 可以支持几个tab下面的列表滑动手势的解析，及滑动动画
 * 每个list作为这个ViewGroup的一个child
 */
...
//	private float mDurationCorr = (float) (Math.PI/1000.0f); // 时间的调和数
	...
	public boolean isDismissParentTouchEvent = false;
...
//			TLog.v(TAG, "new:" + duration + ", old:" + Math.abs(delta) * 2 + ", duration1:" + duration1);
	...
	protected void snapToLastScreen() {
		...
		if (mScrollX != (childCount - 1) * getWidth()) {
			...
			int delta = 0;
//			TLog.v(TAG, "snapToLastScreen, x:" + mScrollX);
...
//			TLog.v(TAG, "snapToLastScreen, delta:" + delta);
//			int duration = (int) (1000 * Math.atan(Math.abs(mDurationCorr * delta))); // 0ms-1500ms
			int duration = (int) (1000 * Math.atan(Math.abs(Math.PI * delta / 1000.0f))); // 0ms-1500ms
//			TLog.v(TAG, "new:" + duration + ", old:" + Math.abs(delta) * 2 + ", duration1:" + duration1);
			mScroller.startScroll(mScrollX, 0, delta, 0, duration/*Math.abs(delta) * 2*/);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if( getParent() != null){
				getParent().requestDisallowInterceptTouchEvent(isDismissParentTouchEvent);
			}
...
//			TLog.v(TAG, "velocityX:" + velocityX + ", mCurScreen:" + mCurScreen);
			...
			if (velocityX > SNAP_VELOCITY && mCurScreen >= 0) {
				...
				if (mIsCircle && left < 0) {
					left += getChildCount();
					snapToLastScreen();
//					TLog.v(TAG, "snap last,");
				}else{
					snapToScreen(left, true);
//					TLog.v(TAG, "snap left," + left);
				}
				...
				if (mIsCircle && right > getChildCount() - 1) {
					right -= getChildCount();
					snapToFirstScreen();
//					TLog.v(TAG, "snap first,");
				}else{
					snapToScreen(right, true);
//					TLog.v(TAG, "snap right, " + right);
				}
			} else {
				snapToDestination();
			}
...
//			break;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		if( mLockAllWhenTouch ){
		    ...
		    // 要递归下逐层锁定，直到ViewPager层面，否则会在低端机出现锁不住的情况
			ViewParent parent = getParent();
			while (parent != null && !(parent instanceof android.support.v4.view.ViewPager)){
			    //XLog.d("million","getParent()="+parent);
			    parent.requestDisallowInterceptTouchEvent(isDismissParentTouchEvent);
...
//		int restoreCount = 0;
		...
		if (fastDraw) {
//			Log.v(TAG, "dispatchDraw, fastDraw");
			...
			if ( Math.abs(scrollPos - leftScreen)> 0.01 && isScreenNoValid(rightScreen)) {
