...
public class TagLayout extends ViewGroup {
    ...
    private int verticalSpacing = 30;   //垂直间距
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //获取用于实际比较的宽度，就是除去2边的padding的宽度
        int noPaddingWidth = width-getPaddingLeft()-getPaddingRight();

        //遍历所有的子View，拿子View的宽和noPaddingWidth进行比较
        Line line = new Line();//准备Line对象
        for (int i = 0; i < getChildCount(); i++) {
            View childView = getChildAt(i);
            childView.measure(0, 0);//保证能够获取到宽高

            //如果当前line中木有子View，则不用比较直接放入line中，因为要保证每行至少有一个子View;
            if(line.getViewList().size()==0){
                line.addLineView(childView);//直接存入
            } else if (line.getLineWidth() + horizontalSpacing + childView.getMeasuredWidth() > noPaddingWidth || line.getViewList().size() == 3) {

                //如果当前line的宽+水平间距+子View的宽大于noPaddingWidth, 或者 一行view已经有3个 则child需要换行
                ...
                //说明当前child应该放入当前Line中
                line.addLineView(childView);
            }

            //如果当前child是最后的子View，那么需要保存最后的line对象
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < lineList.size(); i++) {
            ...
            //计算左侧留白
            int perSpacing = remainSpacing/2;
            ...
            for (int j = 0; j < viewList.size(); j++) {
                ...
                if(j==0){
                    ...
                    //如果是每行的第一行，直接靠左边摆放 加上留白
                    childView.layout(paddingLeft+perSpacing,paddingTop,paddingLeft+perSpacing+childView.getMeasuredWidth(),
