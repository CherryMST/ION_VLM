...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener {
    ...
    private static final int DEFAULT_GREGORIAN_COLOR = 0xffe7e7e7;
    ...
    private int year_start = YEAR_START;
    private int year_end = YEAR_STOP;
    private int year_range = YEAR_SPAN;
    ...
    private void setConfigs(Calendar c, boolean isGregorian, boolean anim) {
        ...
        if (!checkCalendarAvailable(c, year_start, year_end, isGregorian)) {
            c = adjustCalendarByLimit(c, year_start, year_end, isGregorian);
        }
    ...
    public void setNewValuesForAll(int year, int month, int day) {
    ...
    private void setDisplayData(boolean isGregorian) {
        ...
        if (isGregorian) {
            if (mDisplayYearsGregorian == null) {
                mDisplayYearsGregorian = new String[year_range];
            ...
            if (mDisplayMonthsGregorian == null) {
                ...
                for (int i = 0; i < MONTH_SPAN_GREGORIAN; i++) {
                    mDisplayMonthsGregorian[i] = String.valueOf((MONTH_START_GREGORIAN + i < 10 ? "0" + (MONTH_START_GREGORIAN + i) : MONTH_START_GREGORIAN + i));
                }
            }
            if (mDisplayDaysGregorian == null) {
                mDisplayDaysGregorian = new String[DAY_SPAN_GREGORIAN];
                for (int i = 0; i < DAY_SPAN_GREGORIAN; i++) {
                    mDisplayDaysGregorian[i] = String.valueOf((DAY_START_GREGORIAN + i < 10 ? "0" + (DAY_START_GREGORIAN + i) : DAY_START_GREGORIAN + i));
                }
            }
        } else {
            if (mDisplayYearsLunar == null) {
                mDisplayYearsLunar = new String[year_range];
                for (int i = 0; i < year_range; i++) {
    ...
    private void initValuesForY(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        if (isGregorian) {
            ...
            setValuesForPickerView(mYearPickerView, yearSway, year_start, year_end, mDisplayYearsGregorian, false, anim);
        } else {
            int yearSway = cc.get(ChineseCalendar.CHINESE_YEAR);
            setValuesForPickerView(mYearPickerView, yearSway, year_start, year_end, mDisplayYearsLunar, false, anim);
        }
    }
    ...
    public void setGregorian(boolean isGregorian, boolean anim) {
        ...
        if (!checkCalendarAvailable(cc, year_start, year_end, isGregorian)) {
            cc = (ChineseCalendar) adjustCalendarByLimit(cc, year_start, year_end, isGregorian);//调整改变后的界面的显示
        }
